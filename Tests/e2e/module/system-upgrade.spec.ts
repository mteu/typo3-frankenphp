 ...
 * but for the Upgrade module. They share BackendModuleController; if the
 ...
 */
test('Upgrade module: no "Install Tool session expired" message', async ({page}) => {
    test.setTimeout(60_000);
    await page.goto('/typo3/main');
    await page.locator('#modulemenu a[data-moduleroute-identifier]').first()
        .waitFor({state: 'attached', timeout: 30_000});

    await page.locator('#modulemenu a[data-moduleroute-identifier="system_upgrade"]').click();
